...
// 1. Appends the session cost to the token StatsLine (conversation.composer.dock).
// 2. Registers a "消耗" tab (conversation.view) with summary cards, a per-model
...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function ConsumptionView(props) {
      ...
      return jsx(
        "div",
        { style: { padding: 16, display: "flex", flexDirection: "column", gap: 16, fontSize: 13, maxWidth: 860 }, children: [
    ...
    function apply(ctx) {
      ctx.slots.inject("conversation.composer.dock", function () {
        return ctx.slots.register(
          { name: "conversation.composer.dock", id: "billing", order: 1, locale: "billing" },
          BillingCostLine
        );
      });
